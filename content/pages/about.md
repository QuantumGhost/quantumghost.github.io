---
slug: about
status: published
title: 关于我
---

[English Version](/pages/about-en)

- 后端工程师
- 会点 [Python](https://www.python.org)，会点 [Go](https://golang.org/)
- 懂一点密码学

## 联系方式

- 邮件:
    `echo 5Liq5Lq66YKu566x77yabHFkMTk5MUBnbWFpbC5jb20K | base64 --decode`
- [Github](https://github.com/QuantumGhost)
- [Keybase](https://keybase.io/quantumghost)
- [Mastodon: @quantumghost@mastodon.social](https://mastodon.social/@quantumghost)
- [Bluesky: @quantumghost.bsky.social](https://bsky.app/profile/quantumghost.bsky.social)

# 授权协议

除特别注明外，本博客内容均使用 [署名-非商业性使用-相同方式共享 4.0
国际许可协议](https://creativecommons.org/licenses/by-nc-sa/4.0/)
发布，代码均使用 [New BSD
License](https://opensource.org/licenses/BSD-3-Clause) 发布。
